...
void Captura_Dias( int [], int&, int);
void EliminarDatos(int *);
int trab, *ntTrab, empExistentes=0, diasingresar=0;
...
void CapturaDeDatos(int *ntTrab){
    ...
    for(int i=empExistentes;i<*ntTrab+empExistentes;i++){
        ...
        cout<<"Cuantos dias quieres ingresar"<<endl;
        cin>>diasingresar;
        //Funcion captura piezas por dia de la semana
        Captura_Dias(Trabajadores[i].dias, Trabajadores[i].totalpiez, diasingresar);
    }
...
void Captura_Dias(int semana[], int &totalpiezas, int diasingresar){

    totalpiezas=0;
    int day;
    int j;
    for(j=0;j<diasingresar;j++){
        
        
        cout<<"Que dias trabajaste"<<endl;
        cout<<"1.Lunes\n2.Martes\n3.Miercoles\n4.Jueves\n5.Viernes";
        cout<<"Selecciona una opcion: "; cin>>day;
        switch(day){
            case 1:{
                cout<<"Piezas realizadas en el dia Lunes:"<<endl;
                fflush(stdin);
                cin>>(semana[0]);
                break;
            }
            case 2:{
                cout<<"Piezas realizadas en el dia Martes "<<endl;
                fflush(stdin);
                cin>>(semana[1]);
                break;
            }
            case 3:{
                ...
                cout<<"Piezas realizadas en el dia Miercoles "<<endl;
                fflush(stdin);
                cin>>(semana[2]);
                break;
            }
            case 4:{
                cout<<"Piezas realizadas en el dia Jueves"<<endl;
                fflush(stdin);
                cin>>(semana[3]);
                break;
            }
            case 5:{
                cout<<"Piezas realizadas en el dia Viernes"<<endl;
                fflush(stdin);
                cin>>(semana[4]);
                break;
            }
