...
# Obtenga  además la probabilidad del evento 
# {X<9} = {X<=8} =
...
x<- r:8
fx<- choose(n=x-1,k=r-1)*p^r*(1-p)^(x-r)
sum(fx)

pnbinom(q=8-r,size=r,prob=p)   # OJO con esta linea


#pnbinom(q=3-r,size=r,prob=p)  
...
tabla<- cbind(x,fx)
tabla
medias.poblacionales<-colSums(tabla)
medias.poblacionales[1]<-NA
...
tabla
#esperanzas de x,cuadrada,cubica,
medias.poblacionales<-colSums(tabla)
medias.poblacionales[1]<-NA
medias.poblacionales

#varianza
mi<-medias.poblacionales[3]
sigma2<-medias.poblacionales[4]-mi^2
sigma<-sqrt(sigma2)
sesgo<-medias.poblacionales[5]-3*mi*medias.poblacionales[4] +2*mi^3
sesgo<-sesgo/sigma^3
sesgo
c(mi,sigma2,sigma,sesgo)
