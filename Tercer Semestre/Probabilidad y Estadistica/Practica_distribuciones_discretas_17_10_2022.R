# Práctica: distribuciones de probabilidad discretas ejercicio 3.105
# rm(list=ls()) 
# 
# Contenido:
...
fx<- dpois(x=x,lambda=mi) #densidad poison
c(x,fx,f.x)
...
#c)
#Z= numero de autos con defentos en superficie
#Z~bin(n=10, p=0.3935)
n<- 10
p<- 0.3935
#evento de interes = {Z<=1}; a lo mas hay 1 defecto
z<-1
Fz<-pbinom(q=z,size=10,prob=p)
Fz
#por sumatoria
F.z<- cumsum(dbinom(x=c(0,1),size=10,prob=p))
F.z
