...
bool fin(poblacion[],int); // COMPROBAR SI EXISTE UNA SOLUCION
...
bool fin(poblacion final[],int end){ // COMPROBAR SI SE ENCONTRO UNA SOLUCION
	if(final[end].fitness == 0){
		return true;
	}else{
		return false;
	}
...
void mut(poblacion mutar[], int individuo, int reina, int bit){
	...
	mutar[individuo].filCol[filaReina][reina] = 1;
